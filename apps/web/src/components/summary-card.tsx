...
export function SummaryCard({ summary, payers, monthName }: SummaryCardProps) {
  const dollars = Math.floor(summary.grandTotal / 100);
  const cents = summary.grandTotal % 100;
          ...
          {payers.map((payer, index) => {
            const payerTotal =
              summary.totalsByPerson.find((t) => t.paid_by === payer.id)
                ?.total || 0;
            const payerDollars = Math.floor(payerTotal / 100);
            const payerCents = payerTotal % 100;
            const colorClass =
